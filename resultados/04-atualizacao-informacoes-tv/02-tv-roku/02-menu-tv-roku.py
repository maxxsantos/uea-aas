...
broker = 'localhost'
port = 1883
product_id = "0261_1062_6042_3240"
...
def publish_digital_namaplate():
    client = connect_mqtt()
    client.loop_start()
    publish(client, product_id + '/received/digital_nameplate', 'roku-digital-nameplate.json')
    client.loop_stop()

def publish_vendor_informations():
    client = connect_mqtt()
    client.loop_start()
    publish(client, product_id + '/received/vendor_informations', 'roku-vendor-informations.json')
    client.loop_stop()

def publish_technical_data():
    client = connect_mqtt()
    client.loop_start()
    publish(client, product_id + '/received/technical_data', 'roku-technical-data.json')
    client.loop_stop()

def publish_profile():
    client = connect_mqtt()
    client.loop_start()
    publish(client, product_id + '/received/profile', 'roku-profile.json')
    client.loop_stop()
...
def main():
    ...
    submenu_tv_roku = ConsoleMenu("TV Roku",
    ...
    submenu_item_tv_roku = SubmenuItem("TV Roku", submenu=submenu_tv_roku)
    submenu_item_tv_roku.set_menu(menu)

    # Add all the items to the root menu
    # menu.append_item(submenu_item_2)
    menu.append_item(submenu_item_tv_roku)
